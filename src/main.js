...
function clearMoonFocus(){inspectedMoon=null;$('#inspection-subject').hidden=true;for(const row of moonRows.values())row.querySelector('button').setAttribute('aria-pressed','false');}
function inspectMoon(id){
 ...
 $('#inspection-subject').hidden=false;requestRender();
...
function sync(){
 ...
 $('.inspector').dataset.selection=body?'body':'none';
 $('.inspector').setAttribute('aria-label',body?'Selected celestial body':'Free inspection');
 $('#inspector-label').textContent=body?'SELECTED BODY':'FREE INSPECTION';
 if(body){
 $('#body-name').textContent=body.name;
 $('#body-index').textContent=`${String(index+1).padStart(2,'0')} / ${state.pluto?'09':'08'}`;
 $('#body-kind').textContent=body.id==='pluto'?'DWARF PLANET':index<4?'TERRESTRIAL PLANET':index<6?'GAS GIANT':'ICE GIANT';
 $('#body-fact').textContent=body.fact;
 renderBodyPortrait($('#body-portrait'),body);
 $('#body-tilt').textContent=`${Number(body.tiltDeg.toFixed(2))}°`;
 $('#body-eccentricity').textContent=body.eccentricity.toFixed(4);
 $('#body-axis').textContent=fmt(body.aAU,3);
 $('#body-period').textContent=fmt(body.periodDays,1);
 $('#body-radius').textContent=fmt(body.radiusKm,0);
 $('#body-inclination').textContent=fmt(Math.abs(body.inclinationDeg)<0.005?0:body.inclinationDeg,2);
 $('#body-source').href=body.sourceUrl;
 $('#focus-body').replaceChildren(document.createTextNode(`Inspect ${body.name} `),Object.assign(document.createElement('span'),{textContent:'↗'}));
 }else{
 ...
 $('#focus-body').setAttribute('aria-label',body?`Inspect ${body.name}`:state.mode==='mechanical'?'Inspect mechanism':'Frame all planets');
 ...
 $('#distance-model-note').textContent=state.mode==='mechanical'?'Astronomical reference, not the geared arm position.':'Approximate heliocentric position at the selected date.';
 $('#drive-reference').hidden=state.mode!=='mechanical'||!body;
 if(body){
...
function fallback(message){
 ...
 clearMoonFocus();for(const control of document.querySelectorAll('#play,#focus-body,#focus-craft,#view-home,#view-top,#labels,#scale,#speed,#moons,[data-base-style],[data-mode],[data-focus-moon]'))control.disabled=true;
