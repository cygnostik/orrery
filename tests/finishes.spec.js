...
test('opal workmanship can be inspected on the real instrument and returns home cleanly',async({page})=>{
 ...
 await page.getByRole('button',{name:'Inspect opal & finish',exact:true}).click({timeout:5000});
 await expect(page.locator('#inspection-subject')).toContainText('Black opal');
 ...
 await page.locator('#view-home').click();await expect(page.locator('#inspection-subject')).toBeHidden();
...
test('all ten production moons inherit visible mechanical supports and reverse with the crank',async({page})=>{
 await page.goto('/');await expect.poll(()=>page.evaluate(()=>window.__orrery?.diagnostics().ready)).toBe(true);
 await page.locator('#simulation-date').fill('2026-09-05');await page.getByLabel('Include Pluto').check();
