...
test('reduced motion starts still and changing preference stops an explicitly running simulation',async({page})=>{
 ...
 await page.emulateMedia({reducedMotion:'no-preference'});await page.locator('#auto-drive').click();
...
for(const width of [390,640,700,767,768,1024,1440])test(`layout stays bounded at ${width}px`,async({page})=>{
 ...
 await panelAction(page,'Settings',()=>page.locator('#pluto').check());await panelAction(page,'Settings',()=>page.locator('#labels').check());await page.getByRole('button',{name:'09 Pluto',exact:true}).click();await page.waitForTimeout(350);
...
test('distance mapping, labels, and repeated mode changes preserve stable resource counts',async({page})=>{
 await ready(page);await panelAction(page,'Settings',()=>page.locator('#labels').check());await panelAction(page,'Settings',()=>page.getByRole('button',{name:'Observatory',exact:true}).click());await panelAction(page,'Settings',()=>page.locator('#scale').selectOption('distance'));await panelAction(page,'Settings',()=>page.locator('#labels').uncheck());await page.waitForTimeout(350);
 ...
 await panelAction(page,'Settings',()=>page.locator('#pluto').check());await page.waitForTimeout(250);
 // Warm both modes with Pluto: its mechanical supports are uploaded only when first visible.
 for(const mode of ['Mechanical','Observatory']){await panelAction(page,'Settings',()=>page.getByRole('button',{name:mode,exact:true}).click());const f=await page.evaluate(()=>window.__orrery.diagnostics().frame);await page.waitForFunction(frame=>window.__orrery.diagnostics().frame>frame,f);}
 const baseline=await page.evaluate(()=>window.__orrery.diagnostics());
 for(let i=0;i<4;i++){await panelAction(page,'Settings',()=>page.getByRole('button',{name:'Mechanical',exact:true}).click());await panelAction(page,'Settings',()=>page.getByRole('button',{name:'Observatory',exact:true}).click());}
...
test('no WebGL has a meaningful fallback and planet facts still work',async({page})=>{
 ...
 await page.goto('/');await expect(page.locator('#fallback')).toBeVisible();await expect(page.locator('#load-message')).toContainText('3D is unavailable');await expect(page.locator('#auto-drive')).toBeDisabled();await expect(page.locator('#focus-body')).toBeDisabled();
