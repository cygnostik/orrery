...
Planet selection changes an actual surface/cloud specimen portrait, with Saturn ring framing, Uranus axis framing and an explicitly illustrated Pluto, replacing the shared wireframe. Reference obliquity/eccentricity and a selected transmission path provide meaningful detail. Moon inspection replaces the main readouts with the moon's own sourced metadata, retaining the parent system as the renderer selection. Unsupported moon portraits, inclination, Sun distance and geared-year readouts are hidden rather than borrowed from the planet. Desktop inspector content scrolls within the stage height; narrow layouts retain an ordinary document flow. Planet portraits and moon descriptions remain useful without WebGL.
...
- positionAt(id, date): {x,y,z,radiusAU,longitudeDeg} in J2000 ecliptic heliocentric AU. Date JS Date or UTC ISO; x toward equinox, y in ecliptic plane, z north. One long-interval JPL fit supports 1800-01-01 through 2250-01-01 at UTC midnight inclusive; last full date selectable at noon is 2249-12-31. This application boundary follows SCIENCE.range. Invalid inputs throw.
